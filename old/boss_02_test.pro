...
;for i = 1001L, 1016 do begin
    ...
    if (plate[i] ne 9560) or (mjd[i] ne 57805) then begin
        continue
    endif
...
; get data:
    ...
    readspec, plate[i], mjd=mjd[i], flux=flux, $
      ...
      print,'-------------------',i
    if (plate[i] eq 8742) or (mjd[i] eq 57364) then begin
        stop
    endif
;readspec,10243, 58159, zans=zans
; Apply skymask::
...
; with ZWARNING of zero:
    whgal = where( $
     ...
     ;(strtrim(zans.class,2) eq 'GALAXY') and $
